...
<table class="table table-hover" cellspacing="1" id="TablaRes">
    ...
    $SQLgeneral="SELECT E.id AS `id`, E.nombre AS `nombre`, E.apellido AS `apellido`, FORMAT(SUM(T.cantPagado),2) AS `pago`, FORMAT(SUM(T.cantPagadoHrExtra),2) AS `extra` , FORMAT(SUM(T.cantPagado+T.cantPagadoHrExtra),2) AS `total`
        ...
        WHERE T.semana =".$semana." AND E.puesto='".$area."' AND year=".$year." GROUP BY E.apellido ASC
        ";
      ...
      if(@mysql_num_rows($rsP)==0){
        ...
        while($row = mysql_fetch_assoc($rsP)) {
          echo "<tr>";
          echo "<td><a href='detalle-costo-empleado-historial-semana.php?area=".$area."&semana=".$semana."&id=".$row["id"]."&year=".$year."&pagina=".$pagina."'>".ucwords(strtolower($row["apellido"]))." ".ucwords(strtolower($row["nombre"]))."</a></td><td>$ ".$row["pago"]."</td><td>$ ".$row["extra"]."</td><td><strong>$ ".$row["total"]."</strong></td>";
          echo "</tr>";
        }
      }
...
<br/>
<div class="btn-back">
    <a href="historial-semana-costo.php?<?php echo "semana=".$semana."&year=".$year."&pagina=".$pagina; ?>" class="btn btn-default btn-lg" role="button"><span class="glyphicon glyphicon-arrow-left"></span> Regresar</a>
</div>
