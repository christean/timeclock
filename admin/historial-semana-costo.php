...
<table class="table table-hover" cellspacing="1" id="TablaRes">
    ...
    <?php //bucle sql 
     ...
     $SQLgeneral="SELECT E.puesto AS puesto, FORMAT(SUM(T.cantPagado),2) AS `pago`, FORMAT(SUM(T.cantPagadoHrExtra),2) AS `extra` , FORMAT(SUM(T.cantPagado+T.cantPagadoHrExtra),2) AS `total`
        FROM `horas_trabajadas` AS T
        LEFT OUTER JOIN `empleados` AS E ON T.idEmpleado = E.id
        WHERE T.semana=".$numeroSemana." AND T.year=".$year." GROUP BY E.puesto";

      $rsP = mysql_query($SQLgeneral,$con);
   
      if(@mysql_num_rows($rsP)==0){
        echo "No se encontraron Resultados de la semana seleccionada.";
      }else{

        while($row = mysql_fetch_assoc($rsP)) {
          //$totalFila=$row["pago"]+$row["extra"];
          echo "<tr>";
          echo "<td><a href='detalle-costo-area-semana.php?area=".$row["puesto"]."&semana=".$numeroSemana."&year=".$year."&pagina=".$pagina."' >".ucwords(strtolower($row["puesto"]))."</a></td><td>$ ".$row["pago"]."</td><td>$ ".$row["extra"]."</td><td><strong>$ ".$row["total"]."</strong></td>";
          echo "</tr>";
        }
      }
    ?>
...
<br/><br/>
<div class="btn-back">
  <a href="historial-semanas.php?pagina=<?php echo $pagina; ?>" class="btn btn-default btn-lg" role="button"><span class="glyphicon glyphicon-arrow-left"></span> Regresar</a>
</div>
<br/><br/>
</div>
<?php
