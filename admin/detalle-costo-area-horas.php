...
$dia = array('Monday' => 'Lunes', 
 ...
 if ($sqlReport = mysql_query("SELECT dia, horario, hrNormal, hrExtra FROM reporte WHERE idEmpleado=".$idEmpleado." AND semana=".$numeroSemana." AND year=".$year." ",$con)) {
                ...
                while($rowDat = mysql_fetch_assoc($sqlReport)) {
                  ...
                  echo "<tr>
                          <td colspan='2' rowspan='2'><span class='suma'>Suma</span></td><td><strong>".$hrAcumHrNormal." hrs</strong></td><td><strong>".$hrAcumHrExtra." hrs</td><td><strong>".$hrAcumTotal." hrs</strong></td>
                        </tr>
                         <tr>
                          <td><strong>$ ".number_format($normal,2)."</td><td><strong>$ ".number_format($extra,2)."</strong></td><td><strong>$ ".number_format($total,2)."</strong></td>
                        </tr>";
...
<div class="btn-back">
  <a href="detalle-costo-area.php?area=<?php echo $area;?>" class="btn btn-default btn-lg" role="button"><span class="glyphicon glyphicon-arrow-left"></span> Regresar</a>
</div>
<br/><br/>
